...
export default function Home() {
  ...
  async function saveAndContinue() {
    ...
    if (res.ok) { await loadData(); setPage(3); setIsAdmin(false) }
    else alert('Error saving — check console')
    setSaving(false)
  }

  function getTeamData(name: string) { return TEAMS.find(t => t.n === name) }
  ...
  if (loading) return (
    <div style={{display:'flex',alignItems:'center',justifyContent:'center',height:'100vh',background:'#f5f0e8',fontFamily:'sans-serif',fontSize:'1rem',color:'#9a8f7a'}}>
      Loading...
    </div>
  )
    ...
    <div style={{fontFamily:"'Archivo',sans-serif",background:'#f5f0e8',minHeight:'100vh',color:'#1a1508'}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .btn-dark:hover{background:#2d2510;}
        ...
        td{padding:11px 14px;border-bottom:1px solid #ddd5c0;font-size:.85rem;}
        ...
        .modal{background:#fff;border-radius:12px;padding:32px;width:360px;max-width:90vw;}
      `}</style>
      ...
      <header style={{background:'#1a1508',color:'#fff',padding:'0 40px',display:'flex',alignItems:'center',justifyContent:'space-between',height:'58px',position:'sticky',top:0,zIndex:500}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:'12px'}}>
          ...
          {isAdmin && (
            <div style={{display:'flex',alignItems:'center',gap:'4px',fontFamily:"'Roboto Mono',monospace",fontSize:'.65rem',letterSpacing:'.08em',textTransform:'uppercase'}}>
              {[{n:'Players',p:1},{n:'Assign',p:2},{n:'Results',p:3}].map(({n,p})=>(
                <div key={p} onClick={() => setPage(p)} style={{padding:'4px 12px',borderRadius:'4px',cursor:'pointer',color: page===p?'#fff': p<page?'#e8c43a':'rgba(255,255,255,.3)',background: page===p?'rgba(255,255,255,.08)':'transparent',border: page===p?'1px solid rgba(255,255,255,.15)':'1px solid transparent'}}>
                  {p}. {n}
                </div>
              ))}
            </div>
          ...
          {isAdmin
            ? <button className="btn btn-red" style={{padding:'6px 14px',fontSize:'.75rem'}} onClick={adminLogout}>Exit Admin</button>
            ...
            {Array.from({length:16},(_,i)=>(
              <div key={i} style={{background:'#fff',border:'1px solid #ddd5c0',borderRadius:'8px',padding:'10px 12px',display:'flex',alignItems:'center',gap:'8px'}}>
                ...
                <input
                  ...
                  value={playerNames[i]}
                  onChange={e => { const n=[...playerNames]; n[i]=e.target.value.trim(); setPlayerNames(n) }}
                  maxLength={20}
                />
              </div>
            ))}
        ...
        <div style={{display:'flex',flexDirection:'column',minHeight:'calc(100vh - 58px)'}}>
          <div style={{display:'grid',gridTemplateColumns:'1fr 290px',flex:1}}>
                  ...
                  <div key={pot.label} style={{marginBottom:'20px'}}>
                    ...
                    <div style={{display:'flex',flexWrap:'wrap'}}>
                      {potTeams.map(t => {
                        ...
                        return (
                          <div key={t.n} className={`chip${used?' used':''}`} draggable={!used} onDragStart={() => !used && setDragTeam(t.n)}>
            ...
            <div style={{padding:'16px',background:'#f0ebe0',overflowY:'auto',display:'flex',flexDirection:'column',gap:'8px'}}>
              ...
              {activePlayers.map(p => {
                ...
                return (
                  <div key={p} className={`dz${full?' full':''}`}
                    ...
                    <div>
                      {teams.length===0
                        ...
                        : teams.map(tn => {
                            ...
                            return <span key={tn} className="ac" onClick={() => unassign(tn,p)}><span>{td?.f}</span><span>{tn}</span></span>
                          })
                      }
                    </div>
                  </div>
          ...
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'14px 28px',background:'#fff',borderTop:'1px solid #ddd5c0',flexWrap:'wrap',gap:'10px'}}>
            <div style={{fontSize:'.8rem',color:'#9a8f7a'}}>
              {allFull ? <strong style={{color:'#1a1508'}}>All assigned! Ready to save.</strong> : <><strong style={{color:'#1a1508'}}>{needed-assignedCount} teams</strong> left to assign</>}
            </div>
        ...
        <div style={{padding:'28px 36px',maxWidth:'1400px',margin:'0 auto'}}>
          ...
          {noData ? (
            <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',minHeight:'50vh',textAlign:'center',gap:'16px'}}>
              ...
              <div style={{fontSize:'.9rem',color:'#9a8f7a',maxWidth:'400px',lineHeight:1.6}}>
                Setup hasn't happened yet. Get everyone together, hit Admin in the top right, and run the draw.
              </div>
              <div style={{display:'flex',gap:'8px',flexWrap:'wrap',justifyContent:'center',marginTop:'8px'}}>
                <div style={{background:'#fff',border:'1px solid #ddd5c0',borderRadius:'8px',padding:'12px 20px',fontSize:'.82rem',color:'#4a4030'}}>🎲 Enter player names</div>
                <div style={{background:'#fff',border:'1px solid #ddd5c0',borderRadius:'8px',padding:'12px 20px',fontSize:'.82rem',color:'#4a4030'}}>🌍 Drag & drop teams</div>
                <div style={{background:'#fff',border:'1px solid #ddd5c0',borderRadius:'8px',padding:'12px 20px',fontSize:'.82rem',color:'#4a4030'}}>🍺 Track the fines</div>
              </div>
            </div>
          ) : (
            <>
                        ...
                        .map((p,i)=>(
                          <tr key={p.id}>
                            ...
                            <td style={{fontSize:'.78rem'}}>
                              {p.teams.map(tn=>{const td=getTeamData(tn);return <span key={tn} style={{marginRight:'6px'}}>{td?.f} {tn}</span>})}
                            </td>
                            <td className="r">
                              <span style={{display:'inline-flex',alignItems:'center',gap:'4px',background:p.net>0?'#e8f5ee':p.net<0?'#fdecea':'#f0ebe0',color:p.net>0?'#1a6b3a':p.net<0?'#c0392b':'#9a8f7a',border:`1px solid ${p.net>0?'#a0d8b0':p.net<0?'#e8c0c0':'#ddd5c0'}`,borderRadius:'6px',padding:'3px 10px',fontFamily:"'Roboto Mono',monospace",fontSize:'.82rem',fontWeight:700}}>
                                {p.net>0?`+${p.net}`:p.net} 🍺
                              </span>
                            </td>
                          </tr>
                ...
                <div>
                  ...
                  {fines.length===0 ? (
                    <div style={{background:'#fff',border:'1px solid #ddd5c0',borderRadius:'8px',padding:'40px',textAlign:'center',color:'#9a8f7a'}}>
                      ...
                      <div>No fines yet — everyone's behaving themselves.</div>
                    </div>
                  ) : (
                    <table>
                      <thead><tr>
                        <th>Type</th><th>Match</th><th>Detail</th><th>Buys</th><th>Receives</th>
                      </tr></thead>
                      <tbody>
                        {fines.map(f=>(
                          <tr key={f.id}>
                            <td><span style={{fontSize:'1rem'}}>{f.fine_type==='red-card'?'🟥':f.fine_type==='own-goal'?'🤦':'💀'}</span></td>
